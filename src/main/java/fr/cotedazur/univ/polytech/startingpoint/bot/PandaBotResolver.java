...
public class PandaBotResolver {
    ...
    public Action fillObjectivePanda(List<Bambou> bambouSections, List<Bambou> myBambous){
        ...
        if( missingBambous.isEmpty()==false ){
            ...
            return gardenerBotResolver.fillObjectiveGardener(missingBambous.get(missingBambous.size()-1).getBambouType(), false);
        }
        PatternBotResolver patternBotResolver = new PatternBotResolver(map, referee);
        return patternBotResolver.putRandomlyAPLot(bambouSections.get(0).getBambouType());
    }
